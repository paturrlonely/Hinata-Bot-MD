...
global.authFile = `hinata-BotSession`
...
browser: opcion == '1' ? ['HINATA_Bot', 'Edge', '20.0.04'] : methodCodeQR ? ['HINATA-Bot', 'Edge', '20.0.04'] : ["Ubuntu", "Edge", "20.0.04"],
auth: {
creds: state.creds,
...
let prekey = []
let directorio = readdirSync("./HINATA-SubBots")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./HINATA-BotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
let listaDirectorios = readdirSync('./HINATA-SubBots/');
let SBprekey = []
listaDirectorios.forEach(directorio => {
if (statSync(`./HINATA-SubBots/${directorio}`).isDirectory()) {
let DSBPreKeys = readdirSync(`./HINATA-SubBots/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-') /*|| fileInDir.startsWith('app-') || fileInDir.startsWith('session-')*/
})
SBprekey = [...SBprekey, ...DSBPreKeys]
DSBPreKeys.forEach(fileInDir => {
unlinkSync(`./HINATA-SubBots/${directorio}/${fileInDir}`)
})
}
})
...
const directories = ['./HINATA-BotSession/', './HINATA-SubBots/']
